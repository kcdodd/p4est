...
cdef class P8est:
  ...
  cdef _init(P4est self):
    ...
    cdef np.ndarray[np.npy_int32, ndim=3] tree_to_vertex = self._mesh.cells
    ...
    cdef np.ndarray[np.npy_int8, ndim=3] tree_to_edge = self._mesh.cell_adj_edge
    ...
    cdef np.ndarray[np.npy_int8, ndim=1] edge_to_edge = np.ascontiguousarray(self._mesh.edge_cells.flat)
    ...
    self._connectivity.tree_to_tree = <np.npy_int32*>(tree_to_tree.data)
    self._connectivity.tree_to_edge = <np.npy_int8*>(tree_to_edge.data)
  ...
  for root_idx in range(first_local_tree, last_local_tree+1):
    ...
    for q in range(tree.quadrants.elem_count):
      ...
      for k in range(2):
        for j in  range(2):
           for i in  range(2):
            cell_adj_idx = quad_to_quad[cell_idx,k,j,i]
            cell_adj_face_idx = quad_to_face[cell_idx,k,j,i]
            ...
            cell_adj_face[cell_idx,k,j,i] = face_order % 6
            cell_adj_subface[cell_idx,k,j,i] = cell_adj_face_idx // 16
            cell_adj_order[cell_idx,k,j,i] = 1 - 2*(face_order // 6)

            if cell_adj_face_idx >= 0:
              cell_adj[cell_idx,k,j,i] = cell_adj_idx
              cell_adj_level[cell_idx,k,j,i] = 0 if (cell_adj_face_idx < 8) else -1
            else:
              ...
              cell_adj[cell_idx,k,j,i] = quad_to_half[cell_adj_idx]
              cell_adj_level[cell_idx,k,j,i] = 1
